...
export function renderQuizTrend(history, acc) {
  ...
  const trend = sparkData.length >= 2
    ...
    : 0;
  const trendText = trend > 0 ? `▲ +${trend}%` : trend < 0 ? `▼ ${trend}%` : t('view.stats-panel.trendStable');
  ...
  return el('div', { class: 'card quiz-trend', style: { marginTop: '24px' }, dataset: { spotlight: '', reveal: '' } },
    el('div', { class: 'quiz-trend-head' },
      el('div', {},
        el('div', { class: 'card-title' }, t('view.stats-panel.trendTitle')),
        el('div', { class: 'lede', style: { fontSize: '.85rem' } }, t('view.stats-panel.trendSub', { n: sparkData.length, acc }))
      ),
    ...
    sparkData.length >= 2
      ...
      : el('div', { class: 'lede' }, t('view.stats-panel.trendHint')),
